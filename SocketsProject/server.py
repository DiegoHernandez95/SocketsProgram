...
# currently the server does not close when the client session is broken so
# have to kill the terminal to end the program
...
def handleClient(clientSock):
    ...
    # on operating systems. Windows Command prompt uses the command 'dir'
    elif command == "ls":
        if os.name == 'nt':
